...
const Register = ({onLogin, onShowPassword, onTogglePassword}) => {
    ...
    useEffect(()=>{
        ...
        if(pass.match(/([a-z].*[A-Z])|([A-Z].*[a-z])/)){setPassLetter(true)}
        else{setPassLetter(false)}
        
        // Check For Numbers
        if (pass.match(/([0-9])/)){setPassNumber(true)}
        else{setPassNumber(false)}

        // Check For Special char
        if (pass.match(/([!,%,&,@,#,$,^,*,?,_,~])/)) {setPassChar(true)} 
        else {setPassChar(false)}

        if (pass.length > 7) {setPassLength(true)} 
        ...
        <div className="form-container">
            ...
            <div className={showIndicator ? "show-indicator" : "hide-indicator"}>
                ...
                <li className={passNumber ? "pass-green" : "pass-red"}>
                    <span className="--align-center">
                    {passNumber ? <FaCheck /> : <GoDot />}
                    &nbsp; Numbers (0-9)
                    </span>
                </li>
                <li className={passChar ? "pass-green" : "pass-red"}>
                    <span className="--align-center">
                    {passChar ? <FaCheck /> : <GoDot />}
                    &nbsp; Special Character (!@#$%^&*)
                    </span>
                </li>
